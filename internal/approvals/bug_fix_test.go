...
func TestMemoryHeldExecutionStoreCopiesNestedPayload(t *testing.T) {
	...
	if err := store.Save(ctx, execution); err != nil {
		t.Fatalf("save held execution: %v", err)
	}

	nested["count"] = 2
	tags[0] = "changed"

	stored, err := store.Get(ctx, "ap-held")
	if err != nil {
		t.Fatalf("get held execution: %v", err)
	}
	if stored == nil {
		t.Fatal("expected stored held execution")
	}
	storedNested, ok := stored.Payload["nested"].(map[string]any)
	if !ok || storedNested["count"] != 1 {
		t.Fatalf("expected nested payload copy, got %#v", stored.Payload["nested"])
	}
	storedTags, ok := stored.Payload["tags"].([]string)
	if !ok || storedTags[0] != "alpha" {
		t.Fatalf("expected tags payload copy, got %#v", stored.Payload["tags"])
	}

	storedNested["count"] = 3
	storedTags[0] = "mutated"

	again, err := store.Get(ctx, "ap-held")
	if err != nil {
		t.Fatalf("get held execution again: %v", err)
	}
	againNested, ok := again.Payload["nested"].(map[string]any)
	if !ok || againNested["count"] != 1 {
		t.Fatalf("expected stored payload to remain isolated, got %#v", again.Payload["nested"])
	}
	againTags, ok := again.Payload["tags"].([]string)
	if !ok || againTags[0] != "alpha" {
		t.Fatalf("expected stored tags to remain isolated, got %#v", again.Payload["tags"])
	}
}
