...
var (
	...
	dangerousShellCmdPattern  = regexp.MustCompile(`(?i)(?:^|[;&\r\n])\s*(?:rm|cat|curl|wget|bash|sh|nc|netcat|python|perl|powershell|pwsh)\b`)
	dangerousPipeCmdPattern   = regexp.MustCompile(`(?i)\|\s*(?:cat|curl|wget|sh|bash|nc|netcat|python|perl|powershell|pwsh)\b`)
