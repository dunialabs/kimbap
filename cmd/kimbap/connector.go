			...
			if err != nil {
				if outputAsJSON() {
					_ = printOutput(map[string]any{
						...
						"message":    unavailableMessage(componentConnectorState, err),
						"next":       "Configure database in ~/.kimbap/config.yaml and start runtime migrations.",
					})
				}
				return unavailableError(componentConnectorState, err)
			}
			...
			if storeErr != nil {
				if outputAsJSON() {
					_ = printOutput(map[string]any{
						...
						"tenant_id": activeTenant,
						"connector": name,
						"message":   unavailableMessage(componentConnectorState, storeErr),
					})
				}
				return unavailableError(componentConnectorState, storeErr)
			}
