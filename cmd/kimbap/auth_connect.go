...
import (
	...
	"github.com/dunialabs/kimbap/internal/connectors/flows/clientcred"
	"github.com/dunialabs/kimbap/internal/connectors/flows/device"
...
func newAuthReconnectCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if storeErr != nil {
				return unavailableError(componentConnectorStore, storeErr)
			}
		...
		if userInfo, uErr := connectors.FetchUserInfo(ctx, provider.UserInfoEndpoint, accessToken); uErr == nil && userInfo != nil {
			...
			if connectedPrincipal != "" {
				if stored, _ := store.Get(ctx, tenantID, storeName); stored != nil {
					...
					if saveErr := store.Save(ctx, stored); saveErr != nil {
						_, _ = fmt.Fprintf(os.Stderr, "warning: failed to persist connected principal: %v\n", saveErr)
					}
				}
			}
		}
