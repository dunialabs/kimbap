...
# ====================================
# Kimbap-Core One-Click Deployment Script
# ====================================
# This script deploys Go-based Kimbap-Core service from Docker image dunialabs/kimbap-core:latest
# Including PostgreSQL, kimbap-core, and Cloudflared services
...
DEPLOY_DIR=${DEPLOY_DIR:-./kimbap-core-deployment}
...
# Main function
main() {
    log_step "Kimbap-Core One-Click Deployment Script"
    echo ""
    ...
    VOLUME_NAME="postgres_kimbap_core"
...
EOF
    ...
    # Wait for kimbap-core health
    log_info "Waiting for Kimbap-Core to be ready..."
    if wait_for_health "http://localhost:${BACKEND_PORT}/health"; then
        log_success "Kimbap-Core is ready"
    else
        log_error "Kimbap-Core startup failed"
    ...
    echo ""
    echo -e "${GREEN}════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  Kimbap-Core deployed successfully!${NC}"
    echo -e "${GREEN}════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo -e "  2. Deploy Kimbap Console: ${BLUE}https://kimbap.sh/quick-start${NC}"
    echo ""
}
