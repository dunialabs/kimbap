 ...
 * (host.docker.internal → kimbap normalization happens at save time)
 */
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (isConnectionError && url.includes('host.docker.internal')) {
        console.error(
          `[PROXY API] Connection failed to host.docker.internal. If Kimbap Core is running in Docker, try using Docker service name (e.g., kimbap) instead.`,
        );
        console.error(
          `[PROXY API] You can set KIMBAP_CORE_URL environment variable to override, e.g., KIMBAP_CORE_URL=http://kimbap:3002`,
        );
      }
