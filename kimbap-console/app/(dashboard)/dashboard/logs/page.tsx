...
const getStatisticsTimeRange = (range: string) => (
  STATISTICS_SUPPORTED_TIME_RANGES.has(range) ? range : '24h'
)
...
function LogsPageContent() {
  ...
  const [timeFilter, setTimeFilter] = useState<string>(() => {
    ...
    return requested && supportedRanges.has(requested) ? requested : '1h'
  })
  ...
  const clearFilters = () => {
    ...
    setSourceFilter('all')
    setTimeFilter('1h')
    setStatisticsTimeFilter(getStatisticsTimeRange('1h'))
    setSearchTerm('')
    setCurrentPage(1)
  }
