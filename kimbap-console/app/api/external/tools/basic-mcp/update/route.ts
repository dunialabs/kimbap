...
type UpdateServerRequest = Omit<Parameters<typeof updateServer>[1], 'launchConfig' | 'capabilities'> & {
  launchConfig?: string;
  capabilities?: string;
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (server.category !== 1) {
      throw new ExternalApiError(E1003, 'This endpoint only supports Basic MCP tools (category=1)');
    }
    ...
    if (body.authConfig !== undefined) {
      const encryptedConfig = await CryptoUtils.encryptData(JSON.stringify(body.authConfig), ownerToken);
