...
import { getUserByAccessToken, getOwner, connectAllServers } from '@/lib/proxy-api';
  ...
  try {
    ...
    try {
      ...
      await prisma.user.upsert({
        ...
        update: { accessTokenHash: hashToken(tokenToHash), proxyKey: 'single', role: user.role },
        create: { userid: user.userId, accessTokenHash: hashToken(tokenToHash), proxyKey: 'single', role: user.role },
      });
    } catch (saveError) {
